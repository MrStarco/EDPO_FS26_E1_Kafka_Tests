...
GROUP_ID="grp-lag-2-1-a-$(date +%s)"

echo "Resetting environment..."
./reset_env.sh

TOPIC_NAME="click-events-2-1-a-$(date +%s)"
...
echo "Applying Config A (Normal Processing)..."
...
(cd ClickStream-Consumer && mvn clean compile exec:java -Dexec.mainClass="com.examples.ClicksConsumer" -Dtopic.name="$TOPIC_NAME" -Dprocessing.delay=0 > ../$LOG_CONSUMER 2>&1) &
CONSUMER_PID=$!
